            ...
            <div className="hidden sm:inline-flex bg-white dark:bg-slate-900 p-1.5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex-wrap justify-center gap-1">
                {MOODS.map(mood => (
                    <button
                        ...
                        className={`
              px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ease-in-out flex-1 sm:flex-none
              ${activeMood === mood.id
                                ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-md scale-100'
                                : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white hover:scale-[1.03]'}
            `}
                    >
                        {mood.label}
                    </button>
